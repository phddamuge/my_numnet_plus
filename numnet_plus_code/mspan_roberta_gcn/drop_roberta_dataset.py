...
def split_sent_clause(content):
    '''
    split sentence into clause using punctuation
    '''
    pattern = ', |!|\?|;|\.|\. |'
    # pattern = '.|'
    clauses = re.split(pattern, content)
    return clauses


def name_entity_recog(clauses: list):
    nlp = spacy.load('en_core_web_sm')
    useful_entities = []
    useful_clause = []
    useful_key_value_set = []
    for idx, clause in enumerate(clauses):
        clause = nlp(clause)
        # displacy.serve(clause, style='dep')
        # displacy.render(clause,style='dep')
        # ents = clause.ents
        # contain_number = is_contain_number(clause, ents)
        # if contain_number:
        #    useful_clause.append(clause)
        # if len(ents)>0:
        #     ents = (str(item) for item in ents)
        ents_text=[]
        for entity in clause.ents:
            ents_text.append(entity.text)
        useful_entities.append(ents_text)

    return useful_entities


def ner(file_path, output_path):
    ...
    for passage_id, passage_info in tqdm(dataset.items()):
        ...
        passage_clause = split_sent_clause(passage_text)
        useful_passage_entities = name_entity_recog(passage_clause)
        question_ner_result = []
        for question_answer in passage_info["qa_pairs"]:
            question_id = question_answer["query_id"]
            question_text = question_answer["question"].strip()
            question_clause = split_sent_clause(question_text)
            useful_question_entities = name_entity_recog(question_clause)
            question_ner_result.append(
                {"question_id": question_id, "question": question_text, "question_clause": question_clause,
                 "useful_question_entities": useful_question_entities})
        ner_result.append({"passage_id": passage_id, "passage_text": passage_text, "passage_clause": passage_clause,
                           "userful_passage_entities": useful_passage_entities,
                           "question_ner_result": question_ner_result})
        count+=1
        if count >=3:
            print(ner_result)
            with open(output_path, 'w') as f:
                json.dump(ner_result, f, indent=4)
            f.close()
            print('finished ner')
            break
...
if __name__ == '__main__':
    ...
    # sentence = "Among the other denominations there are approximately 93,000 Mormons in 253 congregations, 25,000 Jewish adherents in 21 synagogues, 12,000 Muslims in 39 masjids, 7,000 Buddhists in 34 temples, 7,000 Hindus in 17 temples, 2,500 Unitarian Universalism in 9 congregations, 2,000 Bahai in 17 temples, 5 Sikh temples, a Zoroastrian temple, a Jain temple and an uncounted number of neopagans."
    # sentences = split_sent_clause(sentence)
    # print(sentences)
    # useful_clause = name_entity_recog(sentences)
    # print(useful_clause)

    input_path = '../../numnet_plus_data/drop_dataset/drop_dataset_dev.json'
    output_path = '../../numnet_plus_data/drop_dataset/drop_dataset_dev_ner.json'
    ner(input_path, output_path)
